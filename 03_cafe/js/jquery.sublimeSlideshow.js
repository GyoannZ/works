; (function ($) {
  ...
  let $backgrounds = $("<ul class='sm-slider'></ul>"),
    methods = {
      // Init plugin
      init: function (settings) {
        ...
        let css = this.mkcss(options.src.length, options.duration, options.fade)
      ...
      mkhtml: function (src) {
        ...
        for (let i = 0, length = src.length; i < length; i++) {
          let title = src[i].title ? src[i].title : ""
      ...
      mkcss: function (bgCount, duration, fade) {
        let total_time = duration * bgCount,
          ...
          keyframes =
            "@keyframes imageAnimation {0% {opacity: 0;animation-timing-function: ease-in;}{0}% {opacity: 1;}{1}% {opacity: 1;}{2}% {opacity: 0; animation-timing-function: ease-out;transform: scale(1.17) translateY(-4%) rotateZ(3deg);}100% { opacity: 0;transform: scale(1.17) translateY(-4%) rotateZ(3deg); }} @keyframes titleAnimation {0% { opacity: 0;transform: translateY(-200px); }{0}% { opacity: 1;transform: translateY(0%); }{1}% { opacity: 1;transform: translateY(0%); }{3}% { opacity: 0; transform: scale(0.4) translateY(100%); }100% { opacity: 0; }}".format(
              fadeInPercentile,
              remainPercentile,
              fadeOutPercentile,
        ...
        for (let i = 0; i < bgCount; i++) {
          let bg = src[i]
          css +=
            ".sm-slider li:nth-child({0}) span { background-image: url('{1}');animation-delay: {2}s; } .sm-slider li:nth-child({0}) div { animation-delay: {2}s; }".format(
              i + 1,
              bg.url,
              duration * i
            )
      ...
      addPrefix: function (css) {
        let keyframes = /@(keyframes[\s\S]*?100%.*?\{[\s\S]*?\}[\s\S]*?\})/g,
